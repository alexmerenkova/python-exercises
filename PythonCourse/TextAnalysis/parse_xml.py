...
def rootfind(root, weight):
    weight += 1
    for child in root:
        if child.tag == 'cube' and child.attrib['color'] in result:
            result[child.attrib['color']] += weight
        rootfind(child, weight)

def main():
    root = ElementTree.fromstring(input())
    weight = 1
    if root.tag == 'cube' and root.attrib['color'] in result:
        result[root.attrib['color']] += weight
    rootfind(root, weight)
